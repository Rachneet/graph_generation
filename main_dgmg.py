...
def train_DGMG_epoch(epoch, args, model, dataset, optimizer, scheduler, is_fast = False):
    ...
    for i in order:
        ...
        while node_count<=graph.number_of_nodes():
            node_neighbor = graph.subgraph(list(range(node_count))).adjacency()  # list of lists (first node is zero)
            item = [(n, nbrdict) for n, nbrdict in node_neighbor]
            neighbor_adj = []
            for i in range(len(item)):
                neighbor_adj.append(list(item[i][1]))
            # print(neighbor_adj)
            node_neighbor_new = graph.subgraph(list(range(node_count+1))).adjacency() # list of new node's neighbors
            item_new = [(n, nbrdict) for n, nbrdict in node_neighbor_new]
            neighbor_new = list(item_new[-1][1])

            # 1 message passing
            # do 2 times message passing
            node_embedding = message_passing(neighbor_adj, node_embedding, model)
            ...
            edge_count = 0
            while edge_count<=len(neighbor_new):
                if not is_fast:
                    node_embedding = message_passing(neighbor_adj, node_embedding, model)
                    node_embedding_cat = torch.cat(node_embedding, dim=0)
                    graph_embedding = calc_graph_embedding(node_embedding_cat, model)
                ...
                if edge_count < len(neighbor_new):
                    # calc loss
                    ...
                    # print('neighbor_new',neighbor_new, edge_count)
                    a_node[0,neighbor_new[edge_count]] = 1
                    a_node = Variable(a_node).cuda()
                    # add edge
                    neighbor_adj[-1].append(neighbor_new[edge_count])
                    neighbor_adj[neighbor_new[edge_count]].append(len(neighbor_adj)-1)
                    # calc loss
...
if __name__ == '__main__':
    ...
    # remove self loops
    for graph in graphs:
        edges_with_selfloops = list(graph.selfloop_edges())
