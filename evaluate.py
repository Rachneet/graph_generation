import argparse
from main_dgmg import Args_DGMG
import numpy as np
import os
import re
...
class Args_evaluate():
    def __init__(self):
        ...
        # self.model_name_all = ['vae']
        # self.model_name_all = ['Baseline_DGMG']

        # list of dataset to evaluate
        # use a list of 1 element to evaluate a single dataset
        # self.dataset_name_all = ['primary_school']
        # self.dataset_name_all = ['community2']
        # self.dataset_name_all = ['barabasi_density']
        self.dataset_name_all = ['barabasi_small_completion']
        # self.dataset_name_all = ['barabasi_small']
...
def load_ground_truth(dir_input, dataset_name, model_name='GraphRNN_RNN'):
    ...
    try:
        print("In try of loading ground truth")
...
def evaluation_epoch(dir_input, fname_output, model_name, dataset_name, args, is_clean=True, epoch_start=100,
                     epoch_end=3001, epoch_step=100):
    print('in evaluation_epoch')
    with open(fname_output, 'w+') as f:
        ...
        #     # print("correct test file")
        #     fname_test = dir_input + 'GraphRNN_RNN' + '_' + dataset_name + '_small_4_' + str(64) + '_test_' + str(0) + '.dat'

        elif 'school' in dataset_name:
            print("loading train graphs for test")
            fname_test = dir_input + "GraphRNN_RNN" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_train_' + str(0) + '.dat'
        # elif 'DGMG' in model_name and "community" in dataset_name :
        #     # fname_test = dir_input + "Baseline_DGMG" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
        #     #     hidden) + '_test_' + str(0) + '.p'
        #     fname_test = dir_input + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
        #         hidden) + '_test_' + str(0) + '.dat'

        elif 'completion' in dataset_name:
            print("loading train graphs w/o removed nodes for testing graph completion")
            fname_test = dir_input + "GraphRNN_MLP" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_train_' + str(0) + '.dat'
        # elif 'DGMG' in model_name and "community" in dataset_name :
        #     # fname_test = dir_input + "Baseline_DGMG" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
        ...
        #         hidden) + '_test_' + str(0) + '.dat'
            ...
            fname_test = dir_input + "GraphRNN_RNN" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_test_' + str(0) + '.dat'
            fname_validate = dir_input + "GraphRNN_RNN" + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_validate_' + str(0) + '.dat'
            # fname_test = "/home/rachneet/PycharmProjects/graph_generation/baselines/graphvae/" \
            #              "graphs/barabasi_test_vae_0.dat"
            # fname_validate = "/home/rachneet/PycharmProjects/graph_generation/baselines/graphvae/" \
            #              "graphs/barabasi_validate_vae_0.dat"


        try:
            print('in try of evaluation_epoch')

            # print(fname_test)
            graph_test = load_graph_list(fname_test, is_real=True)
            # graph_validate = load_graph_list(fname_validate, is_real=True)
            print(len(graph_test))
            # print(len(graph_validate))
        ...
        # graph_test = graph_test[int(0.8 * graph_test_len):]  # test on a hold out test set
        #
        # # for completion task, test graph is the training graph
        ...
        if 'GraphRNN' in model_name:
            ...
            for epoch in range(epoch_start, epoch_end, epoch_step):
                for sample_time in range(1, 4):
                    # get filename
                    fname_pred = dir_input + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + \
                                 str(hidden) + '_pred_' + str(epoch) + '_' + str(sample_time) + '.dat'
                    # fname_pred = "./graphs_rnn/" + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                    #     hidden) + '_pred_' + str(epoch) + '_' +str(sample_time) + '.dat'
                    # fname_pred =args.graph_save_path + args.fname_pred + str(epoch) +'_'+str(sample_time) + '.dat'
                    # load graphs
        ...
        if 'Baseline' in model_name or model_name=="vae":
            print("In Baseline pred")
            # read pred graph
            # epoch_start, epoch_end, epoch_step = 0,2100,100
            epoch_start, epoch_end, epoch_step = 0, 1, 2
            # epoch = 0
            #
            for epoch in range(2):
                ...
                # fname_pred = "./baselines/graphvae/graphs/barabasi_small" + '_' + model_name +\
                #              '_' +"new_"+ str(epoch) + '.dat'
                # print(fname_pred)
                # fname_pred = "/home/rachneet/PycharmProjects/dgl_dgmg/graph_pred_community_dgmg_new_"+\
                #              str(epoch)+".dat"
                fname_pred = '/home/rachneet/PycharmProjects/graph_generation/' \
                             'baselines/graphvae/graphs/community_vae_' + str(epoch)+'.dat'
                print(fname_pred)
            #     # load graphs
            # # print(os.path.exists("./graphs/graph_pred_dgmg.dat"))
            # # fname_pred = "./graphs/graph_pred_dgmg.dat"

            # fname_pred = "/home/rachneet/PycharmProjects/dgl_dgmg/graph_pred_community_dgmg_new.dat"
                try:
                    graph_pred = load_graph_list(fname_pred, is_real=True)  # default False
                    #print(graph_pred[0].edges(),graph_pred[1].edges())
                    #print(len(graph_pred[0].edges()),len(graph_pred[1].edges()))
                except:
                    print('Not found: ' + fname_pred)
                    logging.warning('Not found: ' + fname_pred)
                    #continue
...
if __name__ == '__main__':
    ...
    if prog_args.export:
        ...
        if prog_args.graph_type == 'grid':
            ...
            # load from directory
            # real_graph_filename = args.graph_save_path + args.fname_test + '0.dat'
            # for graph completion, loading the original train graphs w/o removed nodes
            real_graph_filename = args.graph_save_path + args.fname_train + '0.dat'
